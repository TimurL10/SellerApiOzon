...
namespace SellerApiOzon.Models
{
    class ApiConnector : IApiConnector
    {
        ...
        string jsonData = "{'category_id': '17028654','language':'DEFAULT'}".Replace("'","\"");
        string jsonData2 = "{'attribute_type': 'ALL','category_id': ['17028654'],'language': 'DEFAULT'}".Replace("'", "\"");
        string  jsonData3  = "{'attribute_id': '8229','category_id': '17038856','language': 'DEFAULT','last_value_id': '1','limit':'5'}".Replace("'", "\"");
        ...
        public void GetSkuFromOzon()
        {
            using (var client = new HttpClient())
            {
                ...
                var response = client.PostAsync(urlWorkEnv + "/v2/product/list", new StringContent(json, Encoding.UTF8, "application/json")).Result;                
                var result = response.Content.ReadAsStringAsync().Result;
